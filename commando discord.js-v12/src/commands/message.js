...
class CommandMessage {
	...
	async run() { // eslint-disable-line complexity
		...
		if(this.command.guildOnly && !this.message.guild) {
			...
			return this.reply(`O comando \`${this.command.name}\` só pode ser usado em servidores.`).then(a => a.delete(5000));
		}

		if(this.command.nsfw && !this.message.channel.nsfw) {
			this.client.emit('commandBlocked', this, 'nsfw');
			return this.reply(`O comando\`${this.command.name}\` só pode ser usado em canais nsfw.`).then(a => a.delete(5000));
		}
		...
		if(!hasPermission || typeof hasPermission === 'string') {
			...
			else return this.reply(`Você não tem permissão para usar o comando \`${this.command.name}\``).then(a => a.delete(5000));
		}
		...
		if(!args && this.command.argsCollector) {
			...
			if(result.cancelled) {
				...
				return this.reply('Comando cancelado.').then(a => a.delete(5000));
			}
			args = result.values;
		}
