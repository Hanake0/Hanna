...
export default class ShopItem {
  ...
  hasVip(user) {
    if(this.client.data.VIPs.has(user.id)) return this.client.data.VIPs.get(user.id).tier;
    return false;
  }
  ...
  defineCurrency(currency) {
    if(this.currency !== currency) {
      ...
      if(currency === 'gems') this.valor /= 1000;
      if(currency === 'coins') this.valor *= 1000;
    };
  }
